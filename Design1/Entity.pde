/**
Entity parent class. The Rabbit and the Predators will be subclasses of it.
**/
public abstract class Entity implements Design {
  ...
  private float _lastTheta; // last angle the Entity sprite was rotated. used to prevent the sprite from always facing the same direction once its destination has been reached
  
  /**
  Constructor
  **/
  public Entity(CoordinateInt position, Double speed, PImage image, World world) {
  ...
  **/
  ...
  /**
  Move an Entity in the direction of its current destination
  **/
  protected void move() {
    ...
    // usually, move one unit of speed of the Entity. but if the destination is closer than one speed unit, limit the speed in order to not overshoot the destination
  ...
  **/
  public void draw(View view) {
    ...
    // angle to draw sprite. if we have reached the destination, use the last angle to avoid the Entity facing the direction of atan2(0, 0) + PI/2
    float theta;
